...
enum
{
	...
	enmDefaultRollingSize		= 300 * 1024 * 1024,//默认分卷大小
	enmDefaultBufSize			= 64 * 1024 * 1024,		//默认缓存大小
...
{
public:
	CLoggerWriter(const char *strLogName);

	CLoggerWriter(const char *strLogName, int32_t nRollingSize);

	virtual ~CLoggerWriter();

	int32_t Debug(const char* szFormat, const char *szFile, int32_t nLineNo, va_list vaList);

	int32_t Info(const char* szFormat, const char *szFile, int32_t nLineNo, va_list vaList);

	int32_t Warn(const char* szFormat, const char *szFile, int32_t nLineNo, va_list vaList);
